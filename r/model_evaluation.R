...
# get sgs ym
sgs.df.ym <- read.csv('model/ym_sgs/output.csv',skip=0,header = T)
...
par(mfrow=c(3,1),
    mar=c(5,5,1,1))
plot(lai~Date,data = hufkens.df.hufken.ym,ylim=c(0,1),pch=16,col = 'red')
legend('topright',legend = c('OBS','CH','SGS'),
       title = 'YM',pch=16,col= c('grey','red','darkseagreen'),bty='n')
# 
points(cover~Date,data = ym.obs.df,pch=16,col='grey')
points(lai~Date,data = sgs.df.ym,pch=16,col='darkseagreen')
points(wtfac_topsoil~Date,data = hufkens.df.hufken.ym,type='l',col='navy')
dev.off()
