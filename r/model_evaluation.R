...
hufkens.df.grass <- read.csv('S:/storage/repo/Mofy/Mofy-master/stp_hufken/output.csv',skip=0,header = T)
...
# get hufkens yanco
hufkens.df.hufken.yanco <- read.csv('S:/storage/repo/Mofy/Mofy-master/yan_hufkens/output.csv',skip=0,header = T)
...
plot.compare.func(hufkens.df.grass,flux.met.df.daily[flux.met.df.daily$Site == 'AU-Stp',])
