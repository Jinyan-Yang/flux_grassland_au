...
make.gday.cfg.func(file.path('model/yanco_spinup','par.cfg'))
# 
...
# 
spin.df <- read.csv(paste0('model/yanco_spinup/',"output.csv"))
...
write.csv(spin.last,'model/yanco_spinup/spin_up_yanco.csv')


# run sim#######
# read spin up 
spin.last <- read.csv('model/yanco_spinup/spin_up_yanco.csv')

#update pools
make.gday.cfg.func(file.path('model/yan_grass','par.cfg'))
make.gday.cfg.func(file.path('model/yan_hufken','par.cfg'))

change_par_func(paste0('model/yan_grass/','par.cfg'),
                'shoot',mean(spin.last$shoot))
change_par_func(paste0('model/yan_grass/','par.cfg'),
                'root',mean(spin.last$root))

change_par_func(paste0('model/yan_hufken/','par.cfg'),
                'shoot',mean(spin.last$shoot))
change_par_func(paste0('model/yan_hufken/','par.cfg'),
                'root',mean(spin.last$root))
